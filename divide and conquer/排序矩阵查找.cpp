...
Line 1038: Char 9: runtime error: reference binding to null pointer of type 'std::vector<int, std::allocator<int> >' (stl_vector.h)
...
bool searchMatrix(vector<vector<int> >& matrix, int target) {
	...
	if(matrix.size()==1)
	{
		j=matrix[0].size()/2;
			...
			while(j>0&&j<matrix[0].size()-1&&low<=high&&j>=low&&j<=high)
			{
				if(matrix[j]==target)
				{
					return true;
				}
				else if(matrix[j]>target)
				{
					high=j-1;
					j=(low+j)/2;
				}
